    ...
    for i in range(120, length, 1):
        ...
        if 보유수량 == 0:
            ...
            if ma60 > ma120 and ma5_1 < ma60_1 and ma5 > ma60: # 60 이평과 120 이평 정배열, 5 이평이 60 이평을 상향돌파할 때
                ...
                진입당최대수익 = 0
                TS최대수익 = 0
                result_df.loc[-1] = ['롱 진입', 일자_시간, 진입가격, 보유수량, 0, 0, 0, round(누적손익,2)]
                result_df.index += 1
                pass
        else:
            청산가 = 종가
            ...
            if 보유수량 > 0 and ma5 < ma60 and ma20 < ma60: # 롱 청산 조건 체크: 5 이평과 20 이평 모두 60 이평 아래로 떨어질 때
                청산명 = '이평청산'
            else:
                if 청산손익 <= -2: # 손절
                    청산명 = '손절'
